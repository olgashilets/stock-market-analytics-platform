...
├── data/
│   ├── source/                  # исходный месячный файл
│   └── generated_archives/      # сгенерированные demo-архивы
├── db/
...
Сгенерированные архивы являются демонстрационными файлами на основе месячных исходных данных. Они используются для проверки ETL, структуры БД и сценариев интерфейса. Это не инвестиционная рекомендация.

## Безопасность

Параметры подключения к БД берутся из переменных окружения. Пароли не захардкожены в коде. Используй `.env.example` как шаблон и не загружай настоящий `.env` на GitHub.
